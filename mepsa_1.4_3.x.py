...
def node_finder (input_matrix, is_4_axes_bool, x_values, y_values, flat_nodes_bool):
    ...
    for global_counter in range (numpy.shape(z_array)[0]):
        ...
        if (max_check == False and (flat_nodes_bool == True or min_check == True)):
            ...
            print(int(minima_counter), x_values[int(x_array[global_counter])], y_values[int(y_array[global_counter])], z_array[global_counter]) #####hqy
        ###hqy###print_progress(global_counter, numpy.shape(z_array)[0]) 
...
def get_points (origin_point, target_point, mask_array, neighbors_array, x_values, y_values, x_array, y_array, z_array, indices_array, error_array):
    mask_position = target_point
    total_out_points = 0
    temp_pos = 0
    temp_array = numpy.empty(numpy.shape(mask_array)[0], dtype = int)
    ...
    if error_array is None:
        out_points = numpy.empty((total_out_points,4))
    else:
        out_points = numpy.empty((total_out_points,6))
    for counter in range(total_out_points):
        ...
        out_points[out_pos][3] = indices_array[temp_array[counter]]
        if error_array is not None:
            out_points[out_pos][4] = error_array[temp_array[counter],0]
            out_points[out_pos][5] = error_array[temp_array[counter],1]
        out_pos -=1
    return out_points
...
def find_path (input_matrix, origin_point, target_point, node_array, points_in_minima_groups, node_matrix_template, is_4_axes_bool, x_values, y_values, if_node_by_node, indices, error):
    print("Preparing")
    input_matrix_shape = numpy.shape(input_matrix)
    if error is None:
        x_array, y_array, z_array, indices_array = generate_three_column_format(get_index_array(numpy.empty(input_matrix_shape[0])), get_index_array(numpy.empty(input_matrix_shape[1])), input_matrix, indices)
        error_array = None
    else:
        print ("Error data detected")
        x_array, y_array, z_array, indices_array, error_array = generate_three_column_format(get_index_array(numpy.empty(input_matrix_shape[0])), get_index_array(numpy.empty(input_matrix_shape[1])), input_matrix, indices, error)
    index_matrix = create_index_matrix(input_matrix, x_array, y_array)
    neighbors_array = create_neighbors_array(index_matrix, x_array, y_array, z_array, is_4_axes_bool)
    ...
    if if_node_by_node == 0:
        out_points = get_points (origin_point, target_point, OT_mask_array, neighbors_array, x_values, y_values, x_array, y_array, z_array, indices_array, error_array)
    else:
        ...
        counter = 1
        print(str(counter) + " of " + str(numpy.shape(final_node_list)[0]-1) + "hqy-1")
        ...
        out_points = get_points (points_in_minima_groups[final_node_list[counter-1]][0], points_in_minima_groups[final_node_list[counter]][0], temp_mask_array, neighbors_array, x_values, y_values, x_array, y_array, z_array, indices_array, error_array)
    
        for counter in range (2, numpy.shape(final_node_list)[0]):
            print(str(counter) + " of " + str(numpy.shape(final_node_list)[0]-1) + "hqy-2")
            mask_gen_bool, temp_mask_array, total_points = mask_gen (points_in_minima_groups[final_node_list[counter-1]][0], points_in_minima_groups[final_node_list[counter]][0], OT_mask_array, z_array, neighbors_array)
            temp_out_points = get_points (points_in_minima_groups[final_node_list[counter-1]][0], points_in_minima_groups[final_node_list[counter]][0], temp_mask_array, neighbors_array, x_values, y_values, x_array, y_array, z_array, indices_array, error_array)
            temp_out_points = numpy.delete(temp_out_points, 0, axis = 0)
            out_points = numpy.append(out_points,temp_out_points, axis=0)
    with open('pathout.pkl', 'wb') as f:
        pickle.dump(out_points, f)
    #numpy.save('path.output', out_points) ###hqy
    print(out_points) #####hqy
...
def surface_plot(xi, yi, x_to_plot, y_to_plot, z_to_plot):
    ...
    grid_xi = numpy.empty((yi.shape[0],xi.shape[0]))
    grid_yi = numpy.empty((yi.shape[0],xi.shape[0]))
    for c in range(yi.shape[0]):
        grid_xi[c,:] = xi
    for c in range(xi.shape[0]):
        grid_yi[:,c] = yi
    zi = griddata((x_to_plot, y_to_plot), z_to_plot, (grid_xi, grid_yi), method='linear')
    ...
    if data_type == "list":
        for arg in args:
            for points_list in arg:
                ...
                if (pre_temp_arg.shape[1] == 6):
                    temp_arg = numpy.empty((numpy.shape(pre_temp_arg)[0],3))
                else:
                    temp_arg = numpy.empty((numpy.shape(pre_temp_arg)[0],1))
                for counter in range (numpy.shape(temp_arg)[0]):
                    temp_arg[counter,0] = pre_temp_arg[counter][2]
                    if (temp_arg.shape[1] == 3):
                        temp_arg[counter,1] = pre_temp_arg[counter][4]
                        temp_arg[counter,2] = pre_temp_arg[counter][5]
                ...
                plt.plot(x,temp_arg[:,0],color = temp_color)
                if (temp_arg.shape[1] == 3):
                    plt.plot(x,temp_arg[:,1],color = temp_color, linewidth = 0.25)
                    plt.plot(x,temp_arg[:,2],color = temp_color, linewidth = 0.25)
                color_counter += 1
    else:
        for arg in args:
            if (len(in_color)>1):
...
def generate_three_column_format(sorted_x_array, sorted_y_array, z_matrix, indices = None, error = None):
    ...
    if indices is not None:
        out_ids = numpy.empty((sorted_x_length[0]*sorted_y_length[0]),dtype=int)
    if error is not None:
        out_error = numpy.empty((sorted_x_length[0]*sorted_y_length[0],2))
    counter_to_plot = 0
    for counter_x in range(0, sorted_x_length[0]):
       for counter_y in range (0, sorted_y_length[0]):
           ...
           if indices is not None:
               out_ids[counter_to_plot] = indices[counter_x][counter_y]
           if error is not None:
               out_error[counter_to_plot,0] = z_matrix[counter_x][counter_y] + error[counter_x][counter_y]
               out_error[counter_to_plot,1] = z_matrix[counter_x][counter_y] - error[counter_x][counter_y]
           counter_to_plot+=1
    if indices is None and error is None:
        return x_to_plot, y_to_plot, z_to_plot
    elif indices is not None and error is None:
        return x_to_plot, y_to_plot, z_to_plot, out_ids
    elif indices is None and error is not None:
        return x_to_plot, y_to_plot, z_to_plot, out_error
    else:
        return x_to_plot, y_to_plot, z_to_plot, out_ids, out_error
...
def three_dimension_sorter(raw_x_array, raw_y_array, raw_z_array, error = None):
    ...
    if raw_x_length[0] <= 0 or raw_x_length[0] != raw_y_length[0] or raw_x_length[0] != raw_z_length[0]:
        ...
        out_z = numpy.empty((1,sorted_x_length[0],sorted_y_length[0]))
        ids = numpy.arange(raw_z_length[0])
        out_ids = numpy.empty((sorted_x_length[0],sorted_y_length[0]))
        if error is not None:
            out_error = numpy.empty((sorted_x_length[0],sorted_y_length[0]))
        else:
            out_error = None
        ...
        for counter_1 in range(0, raw_z_length[0]):
            out_z[0][int(x_index_array[counter_1])][int(y_index_array[counter_1])] = raw_z_array[counter_1]  
            out_ids[int(x_index_array[counter_1])][int(y_index_array[counter_1])] = ids[counter_1]
            if error is not None:
                out_error[int(x_index_array[counter_1])][int(y_index_array[counter_1])] = error[counter_1]
        return sorted_x, sorted_y, out_z, out_error, out_ids
...
class TkMainDialog(tkinter.Frame):
    ...
    def ask_for_input_map(self):
        ...
        if self.file_path != '' and self.file_path != 'none':
            ...
            try:
                try:
                    temp_x, temp_y, temp_z, error = numpy.loadtxt(self.file_path, unpack=True)
                    bad = False
                except:
                ...
                return
            try:
                self.x, self.y, self.maps_list, self.error, self.indices = three_dimension_sorter(temp_x, temp_y, temp_z, error)
            except:
                print((sys.exc_info()))
                tkinter.messagebox.showwarning(
                    "Input map file",
...
#        self.path_generator_top.analyse_global_connectivity_button = tkinter.Button(self.path_generator_top, text="ANALYSE CONNECT.", command=callback_analyse_global_connectivity)
        ...
        def callback_gen_path(mode):
            ...
            if ((self.origin_selection_method == "ARBITRARY" or self.target_selection_method == "ARBITRARY") and if_node_by_node == 1):
                ...
                self.path_not_found, self.path_points, self.visited_points = find_path(self.maps_list[numpy.shape(self.maps_list)[0]-1], self.origin_point, self.target_point, self.node_array, self.points_in_minima_groups, self.node_matrix, self.is_4_axes_bool, self.x, self.y, if_node_by_node, self.indices, self.error)
        ...
        def callback_path_line_plot():
            ...
            if self.path_points.shape[1] == 6:
                line_plot([self.path_points], color = ["red",],data_type = "list")
            else:
                line_plot(self.path_points[:,2], color = ["red",])
        ...
        def callback_save_path():
            ...
            for counter in range(numpy.shape(self.path_points)[0]):
                file_to_save.write(str(int(self.path_points[counter][3])) + " " + str(self.path_points[counter][0]) + " " + str(self.path_points[counter][1]) + " " + str(self.path_points[counter][2]))
                if (self.path_points.shape[1] == 6):
                    file_to_save.write(" " + str(self.path_points[counter][4]) + " " + str(self.path_points[counter][5]))
                file_to_save.write("\n")
            file_to_save.close()
        ...
        def callback_save_stack():
            ...
            for points_list in self.path_points_stack:
                ...
                for counter in range(numpy.shape(temp_array)[0]):
                    file_to_save.write(str(int(temp_array[counter][3])) + " " + str(temp_array[counter][0]) + " " + str(temp_array[counter][1]) + " " + str(temp_array[counter][2]))
                    if (temp_array.shape[1] == 6):
                        file_to_save.write(" " + str(temp_array[counter][4]) + " " + str(temp_array[counter][5]))
                    file_to_save.write("\n")
